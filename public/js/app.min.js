...
(function()
{
    ...
    app.config(['$routeProvider', '$locationProvider', function($routeProvider, $locationProvider)
    {
        $routeProvider
            ...
            .when('/admin/card', {templateUrl: 'views/create_card.html', controller: 'CardController'})
...
(function(){
    ...
    app.controller('CardController', ['$scope', 'Notifications', 'Card', function($scope, Notifications, Card)
    {
        $scope.$emit('setCurrentPage', 'Card');

        $scope.card_user_lastname = null;
        $scope.card_user_firstname = null;
        $scope.card_user_mail = null;

        $scope.showCardModal = function(){
            $('#cardModal').modal('show')
        }
      
        $scope.cardSubmit = function(){
            $('#cardModal').modal('hide')
            $('#cardScanModal').modal('show');
        }
        $('#cardScanModal').on('shown.bs.modal', function(){
            Card.create($scope.card_user_lastname, $scope.card_user_firstname, $scope.card_user_mail)
            .then((data) => {
                $('#cardScanModal').modal('hide')
                if(!data.success){
                    Notifications.error(`Erreur dans la création de la carte : ${data.message}`);
                }
                else{
                    if(data.card_id === undefined)
                        Notifications.error(`Erreur dans la création de la carte : le script de lecture semble cassé...`);
                    Notifications.success(`Carte d'ID ${data.card_id} créée ! Propriétaire : ${data.name}`)
                }
               
            })
            .catch((err) => {
                $('#cardScanModal').modal('hide')
                Notifications.error('Erreur dans la création de la carte: ' + err.message);
            })
        });
    }])
})();
...
(function()
{
    ...
    app.controller('AdminProductsController', ['$scope', '$filter', 'Notifications', 'Products', 'Categories', function($scope, $filter, Notifications, Products, Categories)
    {
        // Current page
        ...
        $scope.addProduct_expired_at_value = $filter("date")(Date.now(), "yyyy-MM-dd")
        $scope.addProduct_quantity_min = 20
        $scope.addProductSubmit = function()
        {
            Products.add($scope.addProduct_name, $scope.addProduct_price, $scope.productImage, $scope.addProduct_expired_at, $scope.addProduct_quantity_min, $scope.selectedCategory)
                .then(function()
                {
        ...
        {
            $("#addProduct_image").val('')
            $('#addProduct_category')[0].selectedIndex = 1;
        ...
        $scope.editProduct_quantity_min = null
        $scope.editProduct_expired_at = null
        $scope.editProduct_selectedCategory = null;
        $scope.editProduct = function(product)
        {
            $scope.editProduct_id = product.id
            $scope.editProduct_name = product.name
            $scope.editProduct_price = parseFloat(product.price)
            $scope.editProduct_expired_at = new Date(product.expired_at);
            $scope.editProduct_quantity_min= product.quantity_min;
            $scope.productImage = product.image
            $scope.editProduct_selectedCategory = product.category == null ? "1" : product.category.id.toString();
            $('#editProduct_category')[0].selectedIndex = product.category == null ? "1" : product.category.id.toString();
        ...
        {
            console.log($scope.editProduct_selectedCategory)
            Products.edit($scope.editProduct_id, $scope.editProduct_name, $scope.editProduct_price, $scope.productImage, $scope.editProduct_expired_at, $scope.editProduct_quantity_min, $scope.editProduct_selectedCategory)
                .then(function()
                {
        ...
        {
            $scope.removeProduct_id = product.id
            $scope.removeProduct_name = product.name
            $scope.removeProduct_price = parseFloat(product.price)
            $scope.productImage = product.image
            $('#removeProduct').modal('show')
        }

        $scope.removeProductSubmit = function(){
            Products.remove($scope.removeProduct_id)
            .then(function(){
        ...
        $('#removeProduct').on('shown.bs.modal', function(e){
            $('#removeProduct_name').first().focus();
        })
...
(function()
{
    ...
    app.controller('HomeController', ['$scope', '$filter', 'Notifications', 'Products', 'Purchases', 'Categories', function($scope, $filter, Notifications, Products, Purchases, Categories)
    {
        // Current page
                ...
                .then(function(products)
                {
                    products.forEach(function(product) {
                         ...
                         if(product.category == null){
                            product.category = {id: 0, nom: "Aucune", desc: "Aucune"};
                         }
                    })
        ...
        function loadCategories(){
            Categories.allNotEmpty()
            .then(function(categories){
                 $scope.categories = categories;
                 $scope.categories.push({id: 0, nom: "Aucune catégorie", desc: ""})
            })
        }

        $scope.filterCategory = function(p, categorie){
            return p.id == categorie.id;
        }

        $scope.dt = function(product){
            let date = new Date(product.expired_at);
            return date.getTime();
        }
        
        $scope.isExpired = function(product){
            if(new Date(product.expired_at).getTime() - $scope.datenow < 0)
                return true;

            return false;
        }

        $scope.productExpiredAt = function(product){
            let date = new Date(product.expired_at);
            return $filter("date")(date, "dd/MM/dd");
        }
        // +1 quantity
        $scope.add = function(product)
        {
            if (product.stock > product.quantity){
                product.quantity++
                if(product.stock - product.quantity < product.quantity_min)
                    Notifications.warning(`Attention ! Le produit ${product.name} est bientôt en rupture de stock !!`);
            }
               
            if(new Date(product.expired_at).getTime() - $scope.datenow < 0)
                Notifications.warning(`${product.name} est périmé !! Vérifie bien la date !!!`);
        ...
        $('#scanCardModal').on('shown.bs.modal', function(){
            Purchases.launchScan()
            .then(function(response){
                $scope.card = response;
                $('#scanCardModal').modal('hide');
                $('#scannedCardModal').modal('show');
            })
            .catch(function(err){
                $('#scanCardModal').modal('hide');
                if(err.status == 404){
                    Notifications.error('Aucun utilisateur lié à cette carte.');
                }
                else{
        ...
        $scope.validateCardSubmit = function(){
            Purchases.purchaseCard($scope.products, $scope.card, $scope.getTotal())
            .then(function(response){
                $('#scannedCardModal').modal('hide');
                loadProducts();
                Notifications.success('Encaissement enregistré !');
            })
            .catch(function(err){
		console.log(err);
                Notifications.error("Erreur lors de l'achat : " + err.data.errors);
            })
        }
...
(function()
{
    ...
    app.controller('UsersController', ['$scope', 'Users', 'Notifications', function($scope, Users, Notifications)
    {
        // Current page
        $scope.$emit('setCurrentPage', 'Users')


        $scope.scannedUser = null;

        $('#scanCardModal').on('shown.bs.modal', function(){
            Users.launchScan()
            .then(function(response){
                $scope.scannedUser = response;
                $('#scanCardModal').modal('hide');
            })
            .catch(function(err){
                $('#scanCardModal').modal('hide');
        ...
        $scope.showModalCoin = function(){
            $('#addCoinModal').modal('show');
        }

        $scope.amount = null;
        $scope.addCoinSubmit = function(){
            Users.addCoin($scope.scannedUser, $scope.amount)
            .then(function(response){
                $('#addCoinModal').modal('hide');
            })
            .catch(function(err){
                Notifications.error('Erreur : ' + err.data.message);
            })
        }

        $scope.isScanned = function(){
            return $scope.scannedUser == null;
        }
 ...
 */
 ...
 (function(){
    ...
    app.service('Card', ['$q', '$http', '$httpParamSerializer', function($q, $http, $httpParamSerializer){


        this.create = function(lastname, firstname, mail){
...
(function()
{
    ...
    app.service('Notifications', function()
    {
        ...
        this.warning = function(message){
            var $alert = $('<div class="alert alert-dark red" role="alert"></div>')
            $alert.text(message)
 ...
 */
 ...
 (function(){
    ...
    app.service('Users', ['$q', '$http', '$httpParamSerializer', function($q, $http, $httpParamSerializer){
        ...
        this.addCoin = function($user, $amount){
            return $q(function(resolve, reject){
                $http({
                    method: 'POST',
                    url: '/api/users/addCoin',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
                    data: $httpParamSerializer({
...
(function(){
    ...
    app.service('Categories', ['$q', '$http', '$httpParamSerializer', function($q, $http, $httpParamSerializer){
        ...
        this.allNotEmpty = function(){
            return $q(function(resolve, reject){
                $http({
                    method: 'GET',
                    url: '/api/categories/allNotEmpty'
                })
                .then(function(response){
                    resolve(response.data);
                })
...
(function()
{
    ...
    app.service('Products', ['$q', '$http', '$httpParamSerializer', function($q, $http, $httpParamSerializer)
    {
        ...
        this.add = function(name, price, image, expired_at, quantity_min, category_id)
        {
            return $q(function(resolve, reject)
            {
                // Request
        ...
        this.edit = function(id, name, price, image, expired_at, quantity_min, category_id)
        {
            return $q(function(resolve, reject)
            {
                // Request
...
(function()
{
    ...
    app.service('Purchases', ['$q', '$http', '$httpParamSerializer', function($q, $http, $httpParamSerializer)
    {
        ...
        // Achat carte
        this.purchaseCard = function(items, card, totalPrice){
            return $q(function(resolve, reject){
                $http({
                    method: 'POST',
                    url: '/api/purchases/purchaseCard',
                    headers: {
                        'Content-Type' : 'application/x-www-form-urlencoded',
                    },
                    data: $httpParamSerializer({
